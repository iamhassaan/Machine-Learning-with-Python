...
class NeuralNetwork():
    def __init__(self,
        ...
        self.w_hidden = np.random.rand(n_in + 1, n_hidden).astype(object)
        self.w_out = np.random.rand(n_hidden + 1, n_out).astype(object)
       
        
        
        # initialize the states of the hidden neurons and the output neurons
        self.y_hidden = np.zeros((n_hidden),dtype=object)
        self.y_out = np.zeros((n_out),dtype=object)
    ...
    def backward_pass(self, inputs, targets, learning_rate):
        ...
        output_deltas = np.zeros((self.n_out),dtype=object)
        ...
        for i in range(self.n_out):
            ...
            if targets.size==1:
                output_deltas[i]=(outputs[i]-targets)*sigmoid_derivative(outputs[i]) # in case we have a single target
            else:
                output_deltas[i]=(outputs[i]-targets[i])*sigmoid_derivative(outputs[i]) #in case we have multiple output

            # output_deltas[i] = ...
            ########################################/
        
        # Step 2. Hidden deltas are used to update the weights of the hidden layer
        hidden_deltas = np.zeros((len(self.y_hidden)),dtype=object)
